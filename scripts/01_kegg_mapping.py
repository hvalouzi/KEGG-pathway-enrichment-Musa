...
# This script automatically identifies the repository root from

# its own location and therefore does not depend on the Jupyter

# Notebook **file** variable.
...
TARGET_MAPPING_FILE = (
RESULTS_DIR /
"KEGG_target_uniprot_kegg_mapping.xlsx"
)

BACKGROUND_MAPPING_FILE = (
RESULTS_DIR /
"KEGG_background_uniprot_kegg_mapping.xlsx"
)
...
KEGG_LINK_URL = (
"https://rest.kegg.jp/link/pathway/mus"
)

KEGG_PATHWAY_LIST_URL = (
"https://rest.kegg.jp/list/pathway/mus"
)
...
# UNIPROT → KEGG MAPPING

# ============================================================

def get_uniprot_kegg_mapping(
uniprot_ids,
batch_size=100,
pause=0.2
):
...
uniprot_ids = sorted(
    set(
        x for x in uniprot_ids
        if x
    )
)

print(
    f"Unique UniProt IDs to map: "
    f"{len(uniprot_ids)}"
)
...
```
raise ValueError(
    "Could not identify the UniProt ID column "
    "in the background input file."
)
```
...
raise ValueError(
    "Could not identify the UniProt ID column "
    "in the target input file."
)
```

print(
"\nBackground UniProt column:",
background_uniprot_column
)

print(
"Target UniProt column:",
target_uniprot_column
)
...
background_uniprot = sorted(
set(
x for x in background_uniprot
if x
)
)

target_uniprot = sorted(
set(
x for x in target_uniprot
if x
)
)

print(
"\nUnique background UniProt IDs:",
len(background_uniprot)
)

print(
"Unique target UniProt IDs:",
len(target_uniprot)
)
...
background_mapping.to_excel(
BACKGROUND_MAPPING_FILE,
index=False
)

target_mapping.to_excel(
TARGET_MAPPING_FILE,
index=False
)
...
print(
"\nBackground UniProt proteins:",
len(background_uniprot)
)

print(
"Background unique KEGG genes:",
len(background_kegg_ids)
)

print(
"Background KEGG genes with pathway annotation:",
len(background_with_pathway)
)

print(
"\nTarget UniProt proteins:",
len(target_uniprot)
)

print(
"Target unique KEGG genes:",
len(target_kegg_ids)
)

print(
"Target KEGG genes with pathway annotation:",
len(target_with_pathway)
)

print(
"\nTotal KEGG gene → pathway records:",
len(pathway_df)
)
...
print(
f"{len(target_with_pathway)} target KEGG genes "
f"vs "
f"{len(background_with_pathway)} background KEGG genes"
)
