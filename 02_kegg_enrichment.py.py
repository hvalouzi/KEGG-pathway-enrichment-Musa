...
#
#   Target = 6 KEGG genes
#   Background = 2,014 KEGG genes
#
# The larger initial numbers of mapped KEGG genes are retained
# only as intermediate mapping information and are NOT used as
...
#
# The script automatically identifies the repository root from
# its own location and therefore does not depend on a personal
# Windows directory.
#
# ============================================================
...
# Therefore:
#
#   SCRIPT_DIR  = KEGG-analysis/scripts/
#   PROJECT_DIR = KEGG-analysis/
#
...
if not required_background_columns.issubset(
    background_df.columns
):

    raise ValueError(
        "Background file is missing the required "
        "'kegg_gene' column."
    )
...
if not required_target_columns.issubset(
    ...
    raise ValueError(
        "Target file is missing the required "
        "'kegg_gene' column."
    )


if not required_pathway_columns.issubset(
    pathway_df.columns
):

    raise ValueError(
        "Pathway file must contain both "
        "'gene' and 'pathway' columns."
    )
...
print(
    "\nUnique background KEGG genes:",
    len(background_genes)
)

print(
    "Unique target KEGG genes:",
    len(target_genes)
)

print(
    "Unique KEGG genes with pathway records:",
    pathway_df["gene"].nunique()
)

print(
    "Unique KEGG pathways:",
    pathway_df["pathway"].nunique()
)
...
if missing_from_background:
    ...
    raise ValueError(
        ...
        "The following target KEGG genes are missing "
        "from the background gene set:\n"
        +
        "\n".join(
            sorted(
                missing_from_background
...
# ============================================================
#
# Only genes with KEGG pathway annotations are included.
#
# Therefore:
#
#   Background universe =
...
#       ∩
#       genes with pathway annotations
#
#   Target universe =
#       target KEGG genes
#       ∩
...
# ============================================================
# FINAL STATISTICAL UNIVERSE SUMMARY
# ============================================================
...
print(
    "\nTarget KEGG genes used for enrichment:",
    len(target_universe)
)

print(
    "Background KEGG genes used for enrichment:",
...
if len(target_universe) == 0:
    ...
    raise ValueError(
        ...
        "No target KEGG genes with pathway annotations "
        "were found."
...
if len(background_universe) == 0:
    ...
    raise ValueError(
        ...
        "No background KEGG genes with pathway annotations "
        "were found."

    )


if not target_universe.issubset(
    background_universe
):

    raise ValueError(

        "Target statistical universe is not a subset "
        "of the background statistical universe."
...
# ============================================================
#
# For every KEGG pathway, a 2 × 2 contingency table is created:
#
...
# Background                c               d
#
# A one-sided Fisher's exact test is used to identify
# enrichment of target genes in each pathway.
#
# ============================================================
...
for pathway, genes in pathway_groups.items():
    ...
    # --------------------------------------------------------
    # Genes belonging to this pathway
    # --------------------------------------------------------
...
# ============================================================
#
# BH correction is applied to all pathway-level P-values
# tested in the enrichment analysis.
#
# ============================================================
...
print(
    "Nominally significant pathways (P < 0.05):",
    len(nominal_significant)
)

print(
    "FDR-significant pathways (FDR < 0.05):",
    len(fdr_significant)
)
...
with pd.ExcelWriter(
    ...
    # --------------------------------------------------------
    # Nominally significant pathways
    # --------------------------------------------------------
    ...
    background_df.to_excel(

        writer,

        sheet_name="Background_genes",
